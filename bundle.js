...
/***/ function(module, exports, __webpack_require__) {
	...
	var _resize = __webpack_require__(5);

	var _resize2 = _interopRequireDefault(_resize);

	var _resizeMatched = __webpack_require__(6);

	var _resizeMatched2 = _interopRequireDefault(_resizeMatched);
	...
	var App = React.createClass({
	  ...
	  _updateBoard: function _updateBoard(newState, cb) {
	    this.setState(newState, cb);
	  },
	  getInitialState: function getInitialState() {
	    return { game: new _game2.default({}, this._updateBoard) };
	  },
	  componentDidMount: function componentDidMount() {
	    (0, _resize2.default)();
	    window.onresize = _resize2.default;
	  },
	  componentDidUpdate: function componentDidUpdate() {
	    (0, _resizeMatched2.default)();
	  },
	  _clickCard: function _clickCard(event) {
	    var card = this.state.game.board.checkKey(event.target.getAttribute("value"));
	  ...
	  _cardFace: function _cardFace(card, key) {
	    if (card.matched) {
	      return React.createElement('div', { className: 'card', key: key, onClick: this._clickCard, value: key, style: { visibility: 'hidden' } });
	    } else if (card.flipped) {
	      return React.createElement(
	        'div',
	        { className: 'card', key: key, onClick: this._clickCard, value: key },
	        card.suit + ' ' + card.value
	      );
	    } else {
	      return React.createElement('div', { className: 'card', key: key, onClick: this._clickCard, value: key });
	    }
	  },
	  _renderGameOver: function _renderGameOver() {
	    if (this.state.game.isOver()) {
	      return React.createElement(
	        'div',
	        { id: 'game-over', style: { position: 'fixed' } },
	        'Game Over! ',
	        this.state.game.leader().name,
	        ' Won!',
	        React.createElement(
	          'div',
	          { className: 'play-again', onClick: function () {
	              this.state.game.reset(_resize2.default);
	            }.bind(this) },
	          'Play Again'
	        )
	      );
	  ...
	  render: function render() {
	    ...
	    return React.createElement(
	      ...
	      React.createElement(
	        'div',
	        { id: 'game-board' },
	        this.state.game.board.allKeys().map(function (key) {
	          var card = _this.state.game.board.checkKey(key);
	          return _this._cardFace(card, key);
	        })
	      ),
	      React.createElement(
	        'div',
	        { id: 'complete-board' },
	        this.state.game.players.allPlayers().map(function (player) {
	          return React.createElement(
	            'div',
	            { key: player.name },
	            React.createElement(
	              ...
	              player.name + '  ' + '(' + player.matchedCards.length + ')'
	            ),
	            React.createElement(
	              'div',
	              { className: 'player-completed-container' },
	              player.matchedCards.map(function (cardPair, i) {
	                return React.createElement(
	                  'div',
	                  { className: 'matched-container', key: i },
	                  React.createElement(
	                    'div',
	                    { className: 'card' },
	                    cardPair[0].suit + ' ' + cardPair[0].value
	                  ),
	                  React.createElement(
	                    'div',
	                    { className: 'card' },
	                    cardPair[1].suit + ' ' + cardPair[1].value
	                  )
	                );
	              })
	            )
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _humanPlayer = __webpack_require__(2);
	...
	var _TurnSelector = __webpack_require__(4);

	var _TurnSelector2 = _interopRequireDefault(_TurnSelector);
	...
	var Game = function () {
	  function Game(initializer, updater) {
	  ...
	  _createClass(Game, [{
	    ...
	    value: function isOver() {
	      return this.board.deck.every(function (card) {
	        return card.matched;
	      });
	    }
	  }, {
...
/***/ function(module, exports) {
	...
	var HumanPlayer = function () {
	  ...
	  _createClass(HumanPlayer, [{
	    key: 'addMatchCards',
	    value: function addMatchCards(pair) {
	...
	var Board = function () {
	  ...
	  _createClass(Board, [{
	    ...
	    value: function shuffle() {
	      ...
	      // shuffles and resets card using fisher-yates
	      var deckSize = this.deck.length;
	      KEYS = [];
	      KEYSHASH = {};

	      for (var i = deckSize - 1; i >= 0; i--) {
	        var replaceIdx = randInt(i + 1);
	        var _ref = [this.deck[replaceIdx], this.deck[i]];
	        this.deck[i] = _ref[0];
	        this.deck[replaceIdx] = _ref[1];
	      }

	      this.deck.forEach(function (card, i) {
	        _this2.deck[i].matched = false;
	        _this2.deck[i].flipped = false;
	    ...
	    key: 'flip',
	    value: function flip(cardSpec) {
	      var foundCard = this.deck.find(function (card) {
	    ...
	    key: 'checkValid',
	    value: function checkValid(cardSpec) {
	      var foundCard = this.deck.find(function (card) {
	        return card.suit == cardSpec.suit && card.value == cardSpec.value;
	      });
...
/***/ function(module, exports) {
	...
	var TurnSelector = function () {
	  function TurnSelector(cb) {
	    _classCallCheck(this, TurnSelector);
	  ...
	  _createClass(TurnSelector, [{
	    key: "add",
	    value: function add(data) {
	      var newNode = new LinkNode(data);
	      if (this.start) {
	        // insert the new node behind the start node
	    ...
	    value: function allPlayers() {
	      var players = [this.start.store];
	      var currentPlayer = this.start.next;
	    ...
	    value: function reset() {
	      this.allPlayers().forEach(function (playerNode) {
	...
	var LinkNode = function LinkNode(data) {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _handleResize = function _handleResize() {
	  ...
	  var completeBoard = document.getElementById('complete-board');
	  completeBoard.style.height = window.innerHeight * 0.3 + 'px';

	  Array.prototype.forEach.call(gameBoard.children, function (el, i) {
...
/***/ function(module, exports) {
	...
	var playerNameSpace = window.innerHeight * 0.03;

	var _handleResizeMatched = function _handleResizeMatched() {
	  ...
	  var playerScores = document.getElementsByClassName('player-score');
	  Array.prototype.forEach.call(playerScores, function (playerScore, i) {
	...
	exports.default = _handleResizeMatched;
